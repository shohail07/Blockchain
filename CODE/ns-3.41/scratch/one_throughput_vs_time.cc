...
// Create a vector to store the throughput values over time
std::vector<std::pair<double, double>> throughputValues;
...
// Calculate and store the throughput values over time
for (double t = 0; t <= 10; t += 0.1) {
    ...
    for (auto it = flowStats.begin(); it!= flowStats.end(); ++it) {
        ...
        throughput += stats.rxBytes * 8 / stats.timeLastRxPacket.GetSeconds();
    }
    throughput /= packetsReceived;
    throughputValues.push_back(std::make_pair(t, throughput));
}

// Print throughput data
std::cout << "Average Throughput: " << throughputValues.back().second << " bps" << std::endl;

// Generate graph using gnuplot
std::ofstream plotFile("throughput.plot");
plotFile << "set terminal png\n";
plotFile << "set output 'throughput.png'\n";
plotFile << "set xlabel 'Time (s)'\n";
plotFile << "set ylabel 'Throughput (bps)'\n";
plotFile << "set yrange [0:1000000]\n"; // Set y-range explicitly
plotFile << "plot '-' with lines\n";
for (auto it = throughputValues.begin(); it!= throughputValues.end(); ++it) {
    plotFile << it->first << " " << it->second << "\n";
}
plotFile << "e\n";
plotFile.close();

system("gnuplot -p throughput.plot");
