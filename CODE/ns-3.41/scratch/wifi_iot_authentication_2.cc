...
/*
  ...
  for (uint32_t i = 0; i < devices.GetN(); ++i) {
    NetDeviceContainer deviceNetDevice = p2p.Install(devices.Get(i), router.Get(0));
    QueueDiscContainer queueDiscs = tch.Install(deviceNetDevice);
    Ptr<QueueDisc> queueDisc = queueDiscs.Get(0); // Use Ptr<QueueDisc> instead of QueueDisc*
    queueDisc->SetAttribute("MaxSize", UintegerValue(100)); // Set maximum size of the queue in bytes
  }
*/
  // Set up animation
