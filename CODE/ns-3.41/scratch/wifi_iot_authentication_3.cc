...
int main() {
  ...
  routerToGatewayAddressHelper.Assign(routerNetDevice);

  // Set up static routes
  Ipv4StaticRoutingHelper staticRouting;
  Ptr<Ipv4StaticRouting> staticRoutingGateway = staticRouting.GetStaticRouting(gateway.Get(0)->GetObject<Ipv4>());
  for (uint32_t i = 0; i < devices.GetN(); ++i) {
    Ptr<Ipv4StaticRouting> staticRoutingDevice = staticRouting.GetStaticRouting(devices.Get(i)->GetObject<Ipv4>());
    staticRoutingDevice->SetDefaultRoute(Ipv4Address("10.0.0.1"), 1);
  }
  ...
  // Set up animation
  AnimationInterface anim("wifi_outputs/wifi_iot_authentication_3/wifi_iot_3-animation.xml"); // Create animation interface
  anim.EnablePacketMetadata(true);
  anim.EnableIpv4RouteTracking("wifi_outputs/wifi_iot_authentication_3/wifi_iot_3-routingtable.xml", Seconds(0), Seconds(600), Seconds(1)); // Enable IPv4 route tracking
  ...
  UdpClientHelper udpClientHelper(InetSocketAddress(gateway.Get(0)->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal(), 8080));
  ...
  monitor->SerializeToXmlFile("wifi_outputs/wifi_iot_authentication_3/wifi_iot_3-flowmon.xml", true, true);
