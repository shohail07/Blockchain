...
int main(int argc, char *argv[]) {
    ...
    AnimationInterface anim("extended-lte-04/lte-animation_v2_3_ddos.xml");
...
// Create a DDoS attack application - start
Ptr<OnOffApplication> ddosApp = CreateObject<OnOffApplication>();
ddosApp->SetAttribute("DataRate", DataRateValue(DataRate("100Mbps")));
ddosApp->SetAttribute("PacketSize", UintegerValue(1024));
ddosApp->SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1.0]"));
ddosApp->SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=0.0]"));

// Install the DDoS attack application on the UEs
ApplicationContainer ddosAppContainer;
for (uint32_t u = 0; u < ueNodes.GetN(); ++u) {
    Ptr<OnOffApplication> app = CreateObject<OnOffApplication>(*ddosApp);
    app->SetNode(ueNodes.Get(u));
    ddosAppContainer.Add(app);
}
ddosAppContainer.Start(Seconds(2.0)); // start the DDoS attack after 2 seconds

// Install the DDoS attack application on the additional UEs
for (uint32_t u = 0; u < ueNodes2.GetN(); ++u) {
    Ptr<OnOffApplication> app = CreateObject<OnOffApplication>(*ddosApp);
    app->SetNode(ueNodes2.Get(u));
    ddosAppContainer.Add(app);
}
ddosAppContainer.Start(Seconds(2.0)); // start the DDoS attack after 2 seconds
    
    // System monitor to track CPU and memory usage
    /*
    SystemMonitor systemMonitor;
    systemMonitor.SetNode(remoteHostContainer.Get(0)); // Monitor the remote host node
    systemMonitor.Start(Seconds(2.0)); // start monitoring after 2 seconds
    */
...
Simulator::Run();    
    ...
    monitor->SerializeToXmlFile("extended-lte-04/lte-flowmon_v2_3_ddos.xml", true, true);

    // Get the flow monitor statistics
    FlowMonitor::FlowStats flowStats = monitor->GetFlowStats();
    double packetLoss = flowStats.lostPackets / (flowStats.lostPackets + flowStats.receivedPackets);
    double throughput = flowStats.receivedBytes * 8 / (flowStats.timeLastRxPacket - flowStats.timeFirstTxPacket);
    double delay = flowStats.delaySum / flowStats.receivedPackets;

    // Get the system monitor statistics
    SystemMonitor::SystemStats systemStats = systemMonitor.GetSystemStats();
    double cpuUsage = systemStats.cpuUsage;
    double memoryUsage = systemStats.memoryUsage;

    // Print the DDOS attack metric data
